...
int logorg_modulus_update(inc, val, maxval)
int inc;
int val;
int maxval;
{
...
int logorg_tabular_rottype(maptype, reduntype, rottype, stripeunit)
int maptype;
int reduntype;
int rottype;
int stripeunit;
...
void logorg_parity_rotate_ideal(currlogorg, curr)
logorg *currlogorg;
ioreq_event *curr;
{
   if (curr->flags & READ) {
...
void logorg_parity_rotate_random(currlogorg, curr)
logorg *currlogorg;
ioreq_event *curr;
{
   if (curr->flags & READ) {
   ...
   while (curr->next->devno == curr->devno) {
      curr->next->devno = (int)((double)currlogorg->numdisks * drand48());
   }
}


int logorg_shadowed_get_short_dist(currlogorg, curr, numtocheck, checklist)
logorg *currlogorg;
ioreq_event *curr;
int numtocheck;
int *checklist;
...
int logorg_shadowed_get_short_queue(currlogorg, curr, def)
logorg *currlogorg;
ioreq_event *curr;
int def;
{
...
int logorg_shadowed(currlogorg, curr, numreqs)
logorg *currlogorg;
ioreq_event *curr;
int numreqs;
{
...
/* May want to move head on secondary device */
			  break;
	    case SHADOW_RANDOM:
			  temp->devno += currlogorg->numdisks * (int) (drand48() * (double) currlogorg->copies);
			  break;
...
int logorg_parity_disk(currlogorg, curr, numreqs)
logorg *currlogorg;
ioreq_event *curr;
int numreqs;
{
...
int logorg_parity_rotate_updates(currlogorg, curr, inc)
logorg *currlogorg;
ioreq_event *curr;
int inc;
{
...
void logorg_parity_read_old_sync(currlogorg, curr, numreqs)
logorg *currlogorg;
ioreq_event *curr;
int numreqs;
{
...
void logorg_parity_read_old_nosync(currlogorg, curr, numreqs)
logorg *currlogorg; 
ioreq_event *curr;
int numreqs;
{
...
int logorg_parity_rotate(currlogorg, curr, numreqs)
logorg *currlogorg;
ioreq_event *curr;
int numreqs;
{
...
int logorg_join_seqreqs(reqlist, curr, seqgive)
ioreq_event *reqlist;
ioreq_event *curr;
int seqgive;
{
...
void logorg_parity_table_insert(head, curr)
ioreq_event **head;
ioreq_event *curr;
{
   ioreq_event *temp;
...
void logorg_parity_table_read_old(currlogorg, rowhead, reqlist, opid)
logorg *currlogorg;
ioreq_event *rowhead;
ioreq_event **reqlist;
int opid;
...
void logorg_parity_table_recon(currlogorg, rowhead, reqlist, stripeno, unitno, tableadd, opid)
logorg *currlogorg;
ioreq_event *rowhead;
ioreq_event **reqlist;
int stripeno;
...
void logorg_parity_table_dodeps_sync(currlogorg, curr, redunlist, reqlist)
logorg *currlogorg;
ioreq_event *curr;
ioreq_event **redunlist;
ioreq_event **reqlist;
{
   int i, j;
   depends *depend;
...
void logorg_parity_table_dodeps_nosync(currlogorg, curr, redunlist, reqlist)
logorg *currlogorg;
ioreq_event *curr;
ioreq_event **redunlist;
ioreq_event **reqlist;
...
int logorg_parity_table(currlogorg, curr, numreqs)
logorg *currlogorg;
ioreq_event *curr;
int numreqs;
{
   int i;
   ioreq_event *reqs[MAXDISKS];
   ioreq_event *redunreqs[MAXDISKS];
   int reqcnt = 0;
...
void logorg_create_table_left_sym(currlogorg)
logorg *currlogorg;
{
   int i, j;
   int numdisks;
...
void logorg_create_table(currlogorg)
logorg *currlogorg;
{
   int i;
   int numdisks;
   ...
   currlogorg->table = (tableentry *) malloc(currlogorg->tablestripes * numdisks * sizeof(tableentry));
...
int logorg_check_dependencies(currlogorg, req, curr)
logorg *currlogorg;
outstand *req;
ioreq_event *curr;
{
