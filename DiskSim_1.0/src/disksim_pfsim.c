...
cpu cpus[MAXCPUS];
process *process_livelist = NULL;
process *extra_process_q = NULL;
int	extra_process_qlen = 0;
int	numcpus = 0;
int	curlbolt = 1;
ioreq_event *pendiolist = NULL;
ioreq_event *doneiolist = NULL;
process *synthlist = NULL;
double  pfscale;
double	lastuser = 0.0;
double	idlein = 0.0;
int	idlereset = 0;
...
char * statdesc_timelimitstats		= "Time limit duration";
...
void pf_allocate_process_structs()
{
   int i;
   process *temp = (process *) malloc(ALLOCSIZE);
...
void pf_addtoextra_process_q(temp)
process *temp;
{
...
process *pf_getfromextra_process_q()
{
   ...
   stat_initialize(statdeffile, statdesc_timelimitstats, &temp->readtimelimitstats);
   stat_initialize(statdeffile, statdesc_timelimitstats, &temp->writetimelimitstats);
   stat_initialize(statdeffile, statdesc_timelimitstats, &temp->readmisslimitstats);
   stat_initialize(statdeffile, statdesc_timelimitstats, &temp->writemisslimitstats);
   temp->ioreq = NULL;
   temp->eventlist = NULL;
   temp->space = NULL;
   temp->link = NULL;
...
event * pf_remove_from_process_eventlist(procp)
process *procp;
{
...
void pf_add_to_process_eventlist(procp, new)
process *procp;
event *new;
{
...
void pf_add_to_intrp_eventlist(intrp, new)
intr_event *intrp;
event *new;
{
...
double pf_add_false_idle_time(length)
double length;
{
   double ret = 0.0;
...
void pf_add_to_pendiolist(procp, curr)
process *procp;
ioreq_event *curr;
{
...
if (pf_printhack) {
   ...
   if ((tmp->buf == curr->buf) && (tmp->opid == curr->opid)) {
      ...
      while ((tmp->next) && ((tmp->next->buf != curr->buf) || (tmp->next->opid != curr->opid))) {
         tmp = tmp->next;
      }
...
void pf_add_cswitch_event(procp, newprocp, cpu_ev)
process *procp;
process *newprocp;
cpu_event *cpu_ev;
{
...
double pf_get_time_to_next_process_event(procp)
process *procp;
{
   double tmptime;
...
double pf_get_time_to_next_intr_event(intrp)
intr_event *intrp;
{
   double tmptime;
...
void pf_idle_cpu_recheck_dispq(cpunum)
int cpunum;
{
...
void pf_handle_cswitch_event(curr, cpu_ev)
cswitch_event *curr;
cpu_event *cpu_ev;
{
   process *procp = NULL;
   ...
   if ((idlereset == 0) && ((curr->newprocp == NULL) || (curr->newprocp->idler != 0))) {
      idlein = simtime;
      idlereset = 1;
   } else {
      idlereset = 0;
...
void pf_handle_idleloop_event(curr, cpu_ev)
idleloop_event *curr;
cpu_event *cpu_ev;
{
   process *procp;
...
void pf_handle_sleep_event(curr, cpu_ev)
sleep_event *curr;
cpu_event *cpu_ev;
{
...
void pf_handle_wakeup_event(curr, cpu_ev)
wakeup_event *curr;
cpu_event *cpu_ev;
{
   process *procp;
...
fprintf (outputfile, "Waking up process %d\n", procp->pid);
   ...
   procp = cpu_ev->procp;
   if ((procp) && ((procp->pfflags & PF_SLEEP_FLAG) && (procp->chan == curr->chan))) {
      procp->pfflags |= PF_WAKED;
   }
}


void pf_handle_ioreq_event(curr, cpu_ev)
ioreq_event *curr;
cpu_event *cpu_ev;
{
   process *procp = cpu_ev->procp;
...
void pf_handle_ioacc_event(curr, cpu_ev)
ioreq_event *curr;
cpu_event *cpu_ev;
{
   if (pf_printhack)
...
void pf_handle_io_internal_event(curr, cpu_ev)
ioreq_event *curr;
cpu_event *cpu_ev;
{
   if (pf_printhack)
...
void pf_clock_interrupt_complete(intrp)
intr_event *intrp;
{
...
void pf_handle_intend_event(curr, cpu_ev)
intend_event *curr;
cpu_event *cpu_ev;
{
   intr_event *intrp = cpu_ev->intrp;
...
void pf_handle_synthio_event(curr, cpu_ev)
event *curr;
cpu_event *cpu_ev;
{
/*
...
void pf_handle_event(curr, cpu_ev)
event *curr;
cpu_event *cpu_ev;
{
if (pf_printhack) {
...
fflush(outputfile);
   ...
   switch (curr->type) {
      ...
      case INTEND_EVENT:
			...
			break;
      case SYNTHIO_EVENT:
			pf_handle_synthio_event((intend_event *)curr, cpu_ev);
			curr = NULL;
			break;
      default:
...
void pf_handle_io_intr_arrive(intrp, cpu_ev)
intr_event *intrp;
cpu_event *cpu_ev;
{
   if (pf_printhack) {
...
fprintf (outputfile, "%f\tIO_INTERRUPT cpu %d  cause %d  buf %p\n", simtime, cpu_ev->cpunum, tmp->cause, tmp->buf);
   ...
   io_interrupt_arrive(intrp);
}


void pf_handle_clock_intr_arrive(intrp, cpu_ev)
intr_event *intrp;
cpu_event *cpu_ev;
{
...
fprintf (outputfile, "%f,   CLOCK_INTERRUPT - new lbolt %d\n", simtime, curlbolt);
   ...
   pf_add_to_intrp_eventlist(intrp, new);
...
void pf_handle_intr_event(intrp, cpunum)
intr_event *intrp;
int cpunum;
{
...
fprintf (outputfile, "Kicking off cpu #%d\n", i);
	 ...
	 cpus[i].current->procp->active = 1;
         cpus[i].current->time = cpus[i].scale * pf_get_time_to_next_process_event(cpus[i].current->procp);
...
void pf_param_override(structtype, paramname, paramval, first, last)
int structtype;
char *paramname;
char *paramval;
int first;
int last;
{
   switch (structtype) {
      ...
      case SYNTHIO:    synthio_param_override(paramname, paramval, first, last);
		       break;

      default:         fprintf(stderr, "Unsupported structure type at pf_param_override: %d\n", structtype);
		       exit(0);
   }
}


void pf_readparams(parfile)
FILE *parfile;
{
   ...
   getparam_bool(parfile, "Print per-process stats?", &pf_print_perprocessstats);
   getparam_bool(parfile, "Print per-CPU stats?", &pf_print_percpustats);
   getparam_bool(parfile, "Print all interrupt stats?", &pf_print_intrstats);
   getparam_bool(parfile, "Print sleep stats?", &pf_print_sleepstats);

   fscanf(parfile, "\nGENERAL PROCESS-FLOW PARAMETERS\n");
   fprintf (outputfile, "\nGENERAL PROCESS-FLOW PARAMETERS\n");

   getparam_int(parfile, "Number of processors", &numcpus, 3, 1, (MAXCPUS-1));
   getparam_double(parfile, "Process-Flow Time Scale", &pfscale, 1, (double) 0.0000000001, (double) 0.0);

   fscanf(parfile, "\nSYNTHETIC I/O TRACE PARAMETERS\n\n");
   fprintf (outputfile, "\nSYNTHETIC I/O TRACE PARAMETERS\n\n");

   synthio_readparams(parfile);
}
...
void pf_print_interrupt_stats(startcpu, stopcpu)
int startcpu;
int stopcpu;
{
   int i;
...
void pf_print_pf_stats(startcpu, stopcpu)
int startcpu;
int stopcpu;
{
   int i;
...
void pf_printstats()
{
   ...
   pf_print_pf_stats(0, numcpus);
   pf_print_interrupt_stats(0, numcpus);
